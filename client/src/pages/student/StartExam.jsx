...
const StartExam = () => {
    ...
    useEffect(() => {
        const fetchExam = async () => {
            ...
            try {
                const response = await axiosInstance.get(`/student/course/get/details/${courseId}`);
                ...
                if (courseData?.exam && courseData.exam.questions) {
                    ...
                    setAnswers(courseData.exam.questions.map(() => ({ selectedOptionIndex: -1 })));
    ...
    const submitExam = async (fromTimer = false) => {
        ...
        if (percentage >= 70) {
            ...
            const discountedPrice = Math.floor(originalPrice - (originalPrice * randomDiscount) / 100);
            ...
            <div className="max-w-xl mx-auto mt-10 text-center">
                {eligible ? (
                    <div className="bg-gradient-to-br from-green-50 to-blue-50 dark:from-gray-800 dark:to-gray-900 p-8 rounded-2xl shadow-lg border border-green-100 dark:border-gray-700">
                        ...
                        <h2 className="text-3xl font-bold text-gray-800 dark:text-white mb-4">Exam Completed!</h2>
                        <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">Congratulations! You scored above 70%.</p>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-inner border border-gray-200 dark:border-gray-700 mb-6">
                            ...
                            <div className="flex items-center justify-center space-x-4">
                                <span className="line-through text-red-500 text-xl">₹{originalPrice}</span>
                                <span className="text-2xl font-bold text-green-700 dark:text-green-500">₹{finalPrice}</span>
                            </div>
                        </div>
                    ...
                    <div className="bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-800 dark:to-gray-900 p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                        ...
                        <h2 className="text-3xl font-bold text-gray-800 dark:text-white mb-4">Exam Completed</h2>
                        <p className="text-lg text-gray-600 dark:text-gray-300 mb-2">You scored less than 70%.</p>
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
                <div>
                    <h2 className="text-3xl font-bold text-indigo-700 dark:text-indigo-300">Exam</h2>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Total Questions: {questions.length}</p>
                </div>
            ...
            <div className="space-y-6">
                {questions.map((question, idx) => (
                    <div key={question._id || idx} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-indigo-500 hover:shadow-lg transition-all duration-200">
                        <p className="font-semibold text-lg text-gray-800 dark:text-gray-200 mb-4">
                            <span className="text-indigo-600 dark:text-indigo-400 font-bold">{idx + 1}.</span> {question.questionText}
                        </p>
                        <div className="space-y-3">
                            {question.options.map((option, optionIndex) => (
                                <label
                                    key={optionIndex}
                                    className={`flex items-start p-4 rounded-lg cursor-pointer transition-all duration-150 ${answers[idx]?.selectedOptionIndex === optionIndex
