  ...
  try {
    ...
    for (const studentId of selectedStudentIds) {
      await Notification.create({
        student: studentId,
        message: `Doubt session scheduled on ${new Date(dateTime).toLocaleString()}.\nNote: ${note}`,
    ...
    res.status(200).json({ success: true, message: "Zoom session created", meeting });
  } catch (error) {
    console.error(error);
...
export const getDueSessions = async (req, res) => {
  const sessions = await Notification.find().populate("student");
  res.json({ sessions });
};
